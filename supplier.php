...
if (isset($_REQUEST["submit"])) {
    try{
        ...
        $stmt = null;
        $stmt = $con->prepare("INSERT INTO tbl_supplier (sup_id, sup_name, gst_no, tin_no, cin_no, pan_no, door_no, addr_line1, addr_line2, city_dist, cstate, pincode, mobile_no1, mobile_no2, email_id, created_by, created_dt) 
                        VALUES (:sup_id, :sup_name, :gst_no, :tin_no, :cin_no, :pan_no, :door_no, :addr_line1, :addr_line2, :city_dist, :cstate, :pincode, :mobile_no1, :mobile_no2, :email_id, :created_by, :created_dt)");
        $data = array(
            ":sup_id" => trim($_REQUEST["sup_id"]),
            ":sup_name" => trim($_REQUEST["sup_name"]),
            ":gst_no" => trim($_REQUEST["gst_no"]),
            ":tin_no" => trim($_REQUEST["tin_no"]),
            ":cin_no" => trim($_REQUEST["cin_no"]),
...
if (isset($_REQUEST["update"])) {
    try{
        ...
        $stmt = null;
        $stmt = $con->prepare("UPDATE tbl_supplier SET sup_name=:sup_name, gst_no=:gst_no, tin_no=:tin_no, cin_no=:cin_no, pan_no=:pan_no, door_no=:door_no, addr_line1=:addr_line1, addr_line2=:addr_line2, city_dist=:city_dist, cstate=:cstate, pincode=:pincode, 
...
if (isset($_REQUEST["did"])) {
    $stmt = null;
    $stmt = $con->query("UPDATE tbl_supplier SET del_status=1 WHERE id=" . $converter->decode($_REQUEST["did"]));
...
$sup_no = $dbcon->GetMaxValue('tbl_supplier', 'id', 'del_status', 0) + 1;
$sup_id = 'S'.leadingZeros($sup_no,5);

//---------------------------------edit----------------------------------------
if (isset($_REQUEST["id"])) {
    $rs = $con->query("SELECT * FROM tbl_supplier where id=" . $converter->decode($_REQUEST["id"]));
    if ($rs->rowCount()) {
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        <?php if(isset($_REQUEST["id"])){
                            echo 'Edit - <span style="color: blue;">'. $sup_id .'</span>';
                        }else{
                            echo 'Add - <span style="color: blue;">'. $sup_id .'</span>';
                        }
                        ?>
                        </h3>
                    </div>
					<br />
                    <form id="thisForm" name="thisForm" action="supplier.php" method="post">
                        <div class="box-body">
                            <div class="form-group col-md-4">
                                <!-- <label class="col-sm-4 control-label">Register No. *</label> -->
                                <!-- <label class="col-sm-5 col-form-label">Supplier Name (IN CAPS) <span class="err">*</span></label> -->
                                <label class="col-form-label">Supplier Name (IN CAPS) <span class="err">*</span></label>
                                <input type="hidden" name="sup_id" id="sup_id" value="<?php echo $sup_id; ?>">
                                <input type="text" class="form-control" name="sup_name" id="sup_name" placeholder="Enter the Supplier Name" title="Enter the Supplier Name" autocomplete="off" autofocus="autofocus" onKeyPress="return isCapitalWithSpace(event);" required oninvalid="this.setCustomValidity('Please enter the supplier name...!')" oninput="this.setCustomValidity('')" maxlength="50" value="<?php echo $sup_name; ?>">
                            </div>
                            <div class="form-group col-md-4">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" class="form-control" name="tin_no" id="tin_no" placeholder="Enter the TIN Number" title="Enter the TIN Number" autocomplete="off" maxlength="30" value="<?php echo $tin_no; ?>">
                            </div><div class="form-group col-md-4">
                                <label class="col-form-label">CIN Number </label>
                                <input type="text" class="form-control" name="cin_no" id="cin_no" placeholder="Enter the CIN Number" title="Enter the CIN Number" autocomplete="off" maxlength="30" value="<?php echo $cin_no; ?>">
                            </div>
                            <div class="form-group col-md-4">
                        ...
                        <div class="dt-responsive table-responsive">
                            <table id="example1" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        <th width="12">#</th>
                                        <th width="60" class="text-center">Sup. ID</th>
                                ...
                                <tbody>
                                    <?php
                                    $rs = $con->query("SELECT * FROM tbl_supplier where del_status = 0 order by id");
                                    if ($rs->rowCount()) {
                                        $sno=1;
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $obj->sup_id; ?></td>
                                        <td><?php echo $obj->sup_name; ?></td>
                                        <td class="text-center">
                                            <a href="supplier.php?id=<?php echo $converter->encode($obj->id); ?>" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;
                                            <a href="supplier.php?did=<?php echo $converter->encode($obj->id); ?>" title="Delete" onclick="return confirm('Are You Sure Want To Delete?');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                        </td>
                                    </tr>
                                    <?php
                                            $sno++;
...
$(document).ready(function() {
    ...
    $('#mastersupplierSubNav').addClass('active');
});
</script>
